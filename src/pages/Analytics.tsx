// src/pages/Analytics.tsx - WITHOUT WALLET CONNECTION REQUIRED
...
export default function Analytics() {
  const { isConnected } = useAccount() // tetap ada tapi tidak dipakai untuk blok
  ...
  const [campaigns, setCampaigns] = useState<CampaignStat[]>(() => {
    ...
    if (cached) {
      try {
        ...
        console.log('Loaded campaigns from cache:', parsed.length)
        return parsed
      } catch (e) {
        return []
      }
  ...
  const [registrations, setRegistrations] = useState<RegistrationEvent[]>(() => {
    ...
    if (cached) {
      try {
        const parsed = JSON.parse(cached)
        console.log('Loaded registrations from cache:', parsed.length)
        return parsed
      } catch (e) {
        return []
      }
  ...
  const { data: allCampaigns, refetch: refetchAllCampaigns, isLoading: loadingCampaigns } = useReadContract({
    ...
    query: { enabled: !!FACTORY_ADDRESS }, // tetap enabled meski wallet tidak connect
  })

  const campaignsList = (allCampaigns as string[]) ?? []

  // Fungsi untuk menentukan apakah FCFS campaign sudah selesai
  ...
  // Hitung actual winner count untuk setiap campaign
  ...
  const fetchAllRegistrations = useCallback(async (campaignAddresses: string[]) => {
    ...
    console.log(`Fetched ${allRegistrations.length} registration events`)
    return allRegistrations
  }, [publicClient])

  // Fetch data - ONLY when called manually
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < total; i += batchSize) {
        ...
        setLoadingProgress(Math.floor((completed / total) * 50)) // 50% untuk campaigns
      ...
      console.log(`Loaded ${loadedCampaigns.length} campaigns, ${registrationsData.length} registrations`)
  ...
  // Filter campaigns with data > 0 untuk top lists
  ...
  // CHART DATA - Berdasarkan waktu registrasi (registeredAt)
  const chartData = useMemo(() => {
    ...
    // Count registrations by registeredAt date
  ...
  const stats = useMemo(() => {
    ...
    // Growth berdasarkan registrations
  ...
  // Show loading only on first load (TIDAK CEK isConnected)
  if ((loadingCampaigns || isLoading) && campaigns.length === 0) {
    return (
      <div className="min-h-screen pt-24 flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="w-10 h-10 text-blue-500 animate-spin mx-auto mb-4" />
          <p className="text-gray-400">Loading dashboard...</p>
          <p className="text-xs text-gray-500 mt-2">Fetching campaign data from blockchain</p>
        </div>
      </div>
    )
  }
              ...
              <thead>
                <tr className="border-b border-gray-700 bg-gray-900">
                  ...
                  <th 
                    ...
                    onClick={() => handleSort('registrants')}
                  >
                    <div className="flex items-center gap-1">
                      Registrants
                      {getSortIcon('registrants')}
                    </div>
                  </th>
                  <th 
                    className="text-left py-3 px-4 text-gray-400 font-medium cursor-pointer hover:text-white transition-colors"
